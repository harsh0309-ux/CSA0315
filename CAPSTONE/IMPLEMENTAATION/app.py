...
def process_waitlist(course_id):
    """
    Automatically promotes students from the waitlist
    whenever seats are available.

    Rules:
    1. First student in the queue gets promoted first.
    2. Student is inserted into enrollments as Approved.
    3. Student is removed from waitlist.
    4. Remaining positions are renumbered.
    5. Continues until no seats or no waitlisted students remain.
    """

    # -----------------------------------------------------
    # STEP 1: Remove duplicate waitlist entries
    # for the same student and same course.
    # Keep the earliest waitlist entry.
    # -----------------------------------------------------

    cursor.execute("""
        SELECT
            MIN(waitlist_id) AS keep_id,
            student_id
        FROM waitlist
        WHERE course_id=%s
        GROUP BY student_id
    ...
    available_seats = int(course[0])

    # -----------------------------------------------------
    # STEP 3: Promote students while seats exist
    # -----------------------------------------------------
    ...
    while available_seats > 0:
        ...
        cursor.execute("""
            ...
            FROM waitlist
            WHERE course_id=%s
            ORDER BY position_no ASC, waitlist_id ASC
            LIMIT 1
        """, (course_id,))
        ...
        # -------------------------------------------------
        # Add student directly as APPROVED
        # -------------------------------------------------
    ...
    # -----------------------------------------------------
    # STEP 4: Recalculate waitlist positions
    # -----------------------------------------------------
...
def register_course(course_id):
    ...
    cursor.execute("""
        SELECT waitlist_id, position_no
        FROM waitlist
        WHERE student_id=%s
        AND course_id=%s
        LIMIT 1
    ...
    if existing_waitlist:
        ...
        flash(
            f"You are already in the waitlist for this course "
            f"at position {existing_waitlist[1]}.",
            "warning"
        )
    ...
    # -----------------------------------------------------
        ...
        # Find next position safely
        cursor.execute("""
            SELECT
                COALESCE(MAX(position_no), 0)
            FROM waitlist
            WHERE course_id=%s
        """, (course_id,))
        ...
        flash(
            f"Course is full. You were added to "
...
def drop_course(enrollment_id):
    ...
    try:
        ...
        # -------------------------------------------------
        # IF APPROVED, CREATE ONE AVAILABLE SEAT
        # -------------------------------------------------
...
def promote_waitlisted_students(course_id):
    ...
    # Keep promoting while seats are available
    while available_seats > 0:

        # Get first student in waitlist
        cursor.execute("""
            SELECT
                waitlist_id,
                student_id,
        ...
        if waiting_student is None:
            break

        waitlist_id = waiting_student[0]
        student_id = waiting_student[1]

        # Safety check:
        # Don't create duplicate enrollment
        cursor.execute("""
            SELECT enrollment_id
            FROM enrollments
            WHERE student_id=%s
        ...
        if existing_enrollment:
            ...
            # Remove duplicate waitlist entry
            cursor.execute("""
                DELETE FROM waitlist
                WHERE waitlist_id=%s
            """, (waitlist_id,))
...
def edit_course(course_id):
    ...
    if request.method == "POST":
        ...
        # IMPORTANT:
        # Take the available seats exactly as entered by admin.
        available_seats = int(
        ...
        try:
            ...
            # DO NOT calculate:
            #
            # available_seats = capacity - approved_students
            #
            # The value entered by the admin is used directly.
            # -------------------------------------------------
            ...
            promoted = promote_waitlisted_students(
                course_id
            )
            ...
            if promoted > 0:
                ...
                flash(
                    ...
                    f"{promoted} waitlisted student(s) "
...
def waitlist():
    ...
    # -----------------------------------------------------
    # Clean duplicate entries for all courses
    # -----------------------------------------------------
    ...
    courses_with_waitlist = cursor.fetchall()

    for row in courses_with_waitlist:
